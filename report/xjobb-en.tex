...
This is, however, not as easy as it sounds as the algorithms have dramatic differences in solving time distribution. By displaying the solving times as cumulative distribution functions, this can however be done, see figure~\ref{fig:allTogether}. 
...
This figure is however not very useful when it comes to qualitevly compare the algorithms, mainly because the algorithms where measured on different puzzles, with different unstable and unsolved measurements.
To add to that, the confidence intervalls of the solving times also varied.
This figure is however useful in getting an overview of the algorithms. 
Before the figures comparative properties are totally dismissed, it shall however be mentioned that the backtrack algorithm and the rulebased algorithm may be compared using this figure as they was tested under similar circumstances. This figure for instance shows that the backtrack algorithm is more likely to solve puzzles very fast, that is in less than 0.01 seconds, than the rulebased solver are. Apart from that it also shows what has already been observed regarding the deviations in solving times.
