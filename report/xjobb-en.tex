...
%Generation and parallelizing TODO
...
%TODO DAVID
% kolla igenom då jag har reviderat hela denna sektionen
% Speciellt avsnittet om boltzmann machines måste ses över då jag där har lämnat en del
% kommentarer istället för att ändra direkt.
...
The grid is devided into 9 boxes which each is a 3x3 grid.
Sudoku has only one rule and that is that all regions, that is rows, columns, and boxes, contains the numbers 1-9 exactly ones.\cite{17clueProof}.
In order to be regarded as a proper Sudoku puzzle it is also required that a unique solution exists, a property which can be analyzed by studying solving for all possible solutions.

Different Sudoku puzzles are widely accepted to have different difficulty levels.
Those are not always so easy to classify as there is no easy way of determining difficulty level by simply expecting a grid.
One instead typically have to try to solve the puzzle to determine how difficult it is.
...
The difficulty of a puzzle is not only problematic as it is hard to determine, but also as it is not generally accepted how one rate puzzles.
One might for instance ascertain that puzzles solvable with a set of rules classifies as easy, and that some additional rules gives the puzzles the rating moderate and advanced.
In this study difficulty will however be defined as the solving time for a certain algorithm, meaning that higher solving times means a more difficult puzzle.
Another interesting aspect related to difficulty ratings is that the minimal number of clues in a proper Sudoku puzzle is 17.\cite{17clueProof} 
Since puzzles even if not individually becomes generally more difficult when the clues are reduced this means that some of the most difficult Sudoku puzzles probably is 17-clue puzzles. 

\FloatBarrier
\section{Computational perspective}
Sudoku solving is a research area in computer science and mathematics, with areas such as solving, puzzle difficulty rating and puzzle generation being researched.\cite{stochastic, sudokuConstruct, generation}%TODO vad är det här för källor till höger. Varför är de utkommenterade? \cite{sudokuConstruct} \cite{generation}.
...
%TODO DAVID
% En hel del förklaringar måste förbättras i denna sektion.
% Först och främst så rekommenderar jag att man börjar med att förklara helheten hur själva
% neurala nätverket fungerar och hur lösningen representeras i den.
% Sen är det inte säkert man behöver gå in alltför mycket i detalj på alla formler och så
% men något kanske behövs då man ju måste förstå hur temperaturen fungerar eftersom
% vi använder just det i analysen sen.
% Observera att det inte gör något om denna del blir längre heller då vi har utrymme för det
% så skriv hellre övertydligt än att förvänta sig att läsaren förstår massa saker.

% Va också noga med att förklara begrepp när de introduceras. Är tillexempelt rätt
% förvirrande att första meningen nu bombar en med massa ord som man inte riktigt är
% säker på vad de betyder.

The concept of Boltzmann machines is gradually introduced by beginning with the neuron, 
%Antar att du menar neural networks. Borde kanske stå
network and finally concluding with a discussion on simulation techniques.\\

\begin{figure}[here] 
\centering
\includegraphics[width=10cm]{images/neuron.png}
\caption{A single neuron with showing weighted inputs from other neurons on the left, 
%Vad innebär bias i detta sammanhang. Rätt oklart. Kanske bättre att stycka upp förklaringen i flera meningar så att det hela blir lite tydligare.
a bias threshold $\theta$, and activation function \emph{s}.}
